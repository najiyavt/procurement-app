    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
      ...
      <div className="mb-6">
        ...
        <button
          ...
          className="mt-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-500 transition duration-200"
        >
          Search
        </button>
      </div>
        ...
        {items.length === 0 ? (
          ...
          <div id="print-content">
            <ul className="space-y-4">
              {items.map((item, index) => (
                ...
                >
                  ...
                  <p>
                    <strong>Stock Unit:</strong> {item.stockUnit}
                  </p>
                  <p>
                    <strong>Unit Price:</strong> ${item.unitPrice.toFixed(2)}
                  </p>
                  <p>
                    <strong>Packing Unit:</strong> {item.packingUnit}
                  </p>
                  <p>
                    <strong>Order Qty:</strong> {item.orderQty}
                  </p>
                  <p>
                    <strong>Net Amount:</strong> ${item.netAmount.toFixed(2)}
                  </p>
                </li>
              ))}
            </ul>
            <div className="mt-4 font-bold text-xl text-gray-800">
              Total: $
              {items.reduce((acc, item) => acc + item.netAmount, 0).toFixed(2)}
            </div>
          </div>
        )}
      </div>
      <button
        ...
        className="mt-4 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-500 transition duration-200"
      >
        Export Purchase Order to Excel
      </button>
      <button
        onClick={printPurchaseOrder}
        className="mt-4 bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-500 transition duration-200"
      >
